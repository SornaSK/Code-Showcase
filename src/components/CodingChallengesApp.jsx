    ...
    <div className="min-h-screen flex flex-col bg-gradient-to-b from-gray-800 to-gray-900 text-white">
        ...
        <div className="flex flex-wrap justify-center gap-3 mb-6">
          {challenges.map((challenge) => (
            <button
              ...
              className={`px-3 py-2 text-sm rounded-lg transition duration-300 
                ${
          ...
          {challenges.map(
            (challenge) =>
              activeChallenge === challenge.id && (
                <div
                  key={challenge.id}
                  className="p-3 bg-gray-800 rounded-lg shadow-lg"
                >
                  <challenge.component />
                </div>
              )
      ...
      <footer className="bg-gray-700 text-gray-300 py-4 mt-auto">
        <div className="container mx-auto flex flex-col items-center text-center md:flex-row md:justify-between md:text-left">
          <p className="text-sm mb-2 md:mb-0">© 2024 Sornakala S. All rights reserved.</p>
          <a
            href="https://www.linkedin.com/in/sornakala-s/"
            target="_blank"
            rel="noopener noreferrer"
            className="flex items-center gap-1 text-cyan-400 hover:text-cyan-300 transition duration-200 text-sm"
          >
            <svg
